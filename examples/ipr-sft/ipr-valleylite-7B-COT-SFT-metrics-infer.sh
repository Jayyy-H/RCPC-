...
OUTPUT_FILE="${OUTPUT_FILE:-${PWD}/result.jsonl}"
...
SCORE_MODE="${SCORE_MODE:-answer_token}"
BATCH_SIZE="${BATCH_SIZE:-2}"
COLLECT_CONFIDENCE_FEATURES="${COLLECT_CONFIDENCE_FEATURES:-true}"
LOW_MARGIN_THRESHOLD="${LOW_MARGIN_THRESHOLD:-1.0}"
REPEATED_NGRAM_SIZE="${REPEATED_NGRAM_SIZE:-3}"
FEATURE_LOGIT_CHUNK_SIZE="${FEATURE_LOGIT_CHUNK_SIZE:-32}"
LIMIT="${LIMIT:-}"
...
build_cmd() {
  ...
  cmd=(
  python3 scripts/inference_ipr_metrics_jsonl.py
  ...
  --feature_logit_chunk_size "${FEATURE_LOGIT_CHUNK_SIZE}"
  ...
  if [[ "${COLLECT_CONFIDENCE_FEATURES}" == "true" || "${COLLECT_CONFIDENCE_FEATURES}" == "1" ]]; then
    cmd+=(--collect_confidence_features)
  else
    cmd+=(--no_collect_confidence_features)
  fi
}
