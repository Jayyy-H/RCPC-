...
# Stage 3: low-LR answer-only alignment with small Long/Short CoT replay.

set -x

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
MODEL_PATH="${MODEL_PATH:?Set MODEL_PATH to the Stage-2 curriculum checkpoint}"
TRAIN_FILE="${TRAIN_FILE:?Set TRAIN_FILE to the multi-view JSONL}"
VAL_FILE="${VAL_FILE:-}"
OUTPUT_DIR="${OUTPUT_DIR:-/mnt/bn/chenhaobo-va-data/lrj/checkpoints/valleylite-7b-answer-align-sft}"
...
export CURRICULUM_PRESET="${CURRICULUM_PRESET:-stage3}"
...
export NUM_TRAIN_EPOCHS="${NUM_TRAIN_EPOCHS:-0.25}"
...
export RUN_NAME="${RUN_NAME:-valleylite-7b-ipr-answer-align-sft}"
