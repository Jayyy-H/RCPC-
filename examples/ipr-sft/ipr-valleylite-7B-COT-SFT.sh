...
# Standalone CoT SFT cold-start script.
...
OUTPUT_DIR="${OUTPUT_DIR:-/mnt/bn/chenhaobo-va-data/lrj/checkpoints/valleylite-7b-sft-0607}"
...
WANDB_PROJECT="${WANDB_PROJECT:-ipr-cot-sft}"
RUN_NAME="${RUN_NAME:-valleylite-7b-ipr-cot-sft}"
...
SFT_EVAL_SKIP_BAD_SAMPLES="${SFT_EVAL_SKIP_BAD_SAMPLES:-false}"
...
export PROMPT_KEY TARGET_KEY MAX_PROMPT_LENGTH MAX_RESPONSE_LENGTH MAX_PIXELS MIN_PIXELS
...
export SFT_SKIP_BAD_SAMPLES SFT_EVAL_SKIP_BAD_SAMPLES SFT_MAX_SAMPLE_RETRIES SFT_AUDIT_DATASETS
...
SFT_TRAIN_PY="${TMPDIR:-/tmp}/ipr_cot_sft_train_${USER:-user}_$$.py"
...
class IPRCotSFTDataset(torch.utils.data.Dataset):
    ...
    def _build_item_from_row(self, row: Dict[str, Any], *, index: int) -> Dict[str, Any]:
        ...
        pil_images = [
            process_image(Image.open(path), self.max_pixels, self.min_pixels)
...
def dataset_relationship_stats(dataset: IPRCotSFTDataset):
    ...
    for row in dataset.dataset:
        try:
            ...
            target_digest = stable_digest(row[dataset.target_key])
        except Exception:
            invalid_rows += 1
            continue
        valid_rows += 1
        ...
        target_lengths.append(len(str(row[dataset.target_key])))
    ...
    stats = {
        ...
        "target_chars_mean": float(np.mean(target_lengths)) if target_lengths else 0.0,
        "target_chars_p50": float(np.median(target_lengths)) if target_lengths else 0.0,
        "target_chars_p95": float(np.percentile(target_lengths, 95)) if target_lengths else 0.0,
    }
...
def audit_train_eval_relationship(train_dataset: IPRCotSFTDataset, eval_dataset: IPRCotSFTDataset) -> None:
    ...
    if conflicting_eval_pairs:
        print(
            ...
            "Token-level eval_loss can rise while task quality improves because SFT is fitting a different target style.",
            flush=True,
        )


class IPRCotSFTCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for input_ids, labels, attention_mask in zip(input_ids_list, labels_list, attention_mask_list):
            ...
            batch_labels.append(torch.cat([labels, torch.full((pad_length,), -100, dtype=torch.long)], dim=0))
...
def main() -> None:
    ...
    if processor is None:
        raise RuntimeError("A multimodal processor is required for IPR CoT SFT.")
    ...
    training_kwargs = {
        ...
        "run_name": os.getenv("RUN_NAME", "valleylite-7b-ipr-cot-sft"),
    ...
    trainer_kwargs = {
        ...
        "data_collator": IPRCotSFTCollator(tokenizer),
    }
    if eval_dataset is not None:
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC_PER_NODE}" "${SFT_TRAIN_PY}" "$@"
