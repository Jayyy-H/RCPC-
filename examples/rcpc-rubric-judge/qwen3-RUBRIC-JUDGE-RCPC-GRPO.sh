...
MICRO_BATCH_SIZE_FOR_UPDATE="${MICRO_BATCH_SIZE_FOR_UPDATE:-2}"
MICRO_BATCH_SIZE_FOR_EXPERIENCE="${MICRO_BATCH_SIZE_FOR_EXPERIENCE:-2}"
VAL_BATCH_SIZE="${VAL_BATCH_SIZE:-16}"
MAX_VAL_BATCHES="${MAX_VAL_BATCHES:-3}"
...
SAVE_CKPT_LIMIT="${SAVE_CKPT_LIMIT:-10}"
SAVE_SHARD_LIMIT="${SAVE_SHARD_LIMIT:-10}"
VAL_GENERATIONS_TO_LOG_TO_WANDB="${VAL_GENERATIONS_TO_LOG_TO_WANDB:-10}"
RCPC_BUDGET="${RCPC_BUDGET:-24}"
RCPC_DERIVE_CANDIDATES_FROM_BUDGET="${RCPC_DERIVE_CANDIDATES_FROM_BUDGET:-true}"
RCPC_DYNAMIC_BUDGET_ENABLED="${RCPC_DYNAMIC_BUDGET_ENABLED:-true}"
RCPC_MIN_BUDGET="${RCPC_MIN_BUDGET:-12}"
RCPC_MAX_BUDGET="${RCPC_MAX_BUDGET:-28}"
RCPC_REFERENCE_ACTION_COUNT="${RCPC_REFERENCE_ACTION_COUNT:-0}"
RCPC_CANDIDATE_POOL_MULTIPLIER="${RCPC_CANDIDATE_POOL_MULTIPLIER:-2}"
RCPC_TRANSPORT_LAMBDA="${RCPC_TRANSPORT_LAMBDA:-2.0}"
...
RCPC_COUNTERFACTUAL_SAMPLES="${RCPC_COUNTERFACTUAL_SAMPLES:-4}"
...
# A failed RCPC plan falls back to the ordinary criterion advantage for that
# group. Keep training alive unless strict failure is explicitly requested.
RCPC_FAIL_ON_INTERVENTION_ERROR="${RCPC_FAIL_ON_INTERVENTION_ERROR:-false}"
...
if [[ "${ROPD_SMOKE:-1}" == "1" ]]; then
    EXTRA_ARGS+=(
        ...
        worker.reward.ropd_rcpc_dynamic_budget_enabled=false
...
python3 -m verl.trainer.main \
    ...
    worker.reward.ropd_rcpc_dynamic_budget_enabled="${RCPC_DYNAMIC_BUDGET_ENABLED}" \
    worker.reward.ropd_rcpc_min_budget="${RCPC_MIN_BUDGET}" \
    worker.reward.ropd_rcpc_max_budget="${RCPC_MAX_BUDGET}" \
    worker.reward.ropd_rcpc_reference_action_count="${RCPC_REFERENCE_ACTION_COUNT}" \
    worker.reward.ropd_rcpc_candidate_pool_multiplier="${RCPC_CANDIDATE_POOL_MULTIPLIER}" \
