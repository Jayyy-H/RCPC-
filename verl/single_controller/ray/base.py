...
class RayWorkerGroup(WorkerGroup):
    ...
    def _get_named_actor(self, name: str):
        ...
        for actor_info in self._safe_list_named_actors():
            if actor_info.get("name") != name:
                continue
            namespace = actor_info.get("namespace")
            try:
                if namespace:
    ...
    def _safe_list_named_actors(self):
        try:
            return list_named_actors(all_namespaces=True)
        except Exception as exc:
            return [{"error": repr(exc)}]

    def _get_worker_state_summary(self, worker: ray.actor.ActorHandle):
        try:
            worker_state_dict = get_actor(worker._actor_id.hex()) or {}
        except Exception as exc:
            return {"error": repr(exc)}

        return {
            key: worker_state_dict.get(key)
            for key in ("state", "name", "pid", "node_id", "address")
            if key in worker_state_dict
        }

    def _get_driver_master_addr_port(self) -> Tuple[str, str]:
        master_addr = (
            os.getenv("VERL_MASTER_ADDR")
            or os.getenv("MY_HOST_IP")
            or os.getenv("MY_HOST_IPV6")
            or ray._private.services.get_node_ip_address()
        )
        master_port = os.getenv("VERL_MASTER_PORT")
        if not master_port:
            with socket.socket() as sock:
                sock.bind(("", 0))
                master_port = str(sock.getsockname()[1])
        return str(master_addr), str(master_port)
    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached):
        ...
        precompute_master = os.getenv("VERL_PRECOMPUTE_MASTER", "1").strip().lower() not in {
            "0",
            "false",
            "no",
            "off",
        }
        if precompute_master and (self._master_addr is None or self._master_port is None):
            self._master_addr, self._master_port = self._get_driver_master_addr_port()
        ...
        for pg_idx, local_world_size in enumerate(resource_pool.store):
            ...
            for local_rank in range(local_world_size):
                ...
                if self._master_addr is not None and self._master_port is not None:
                ...
                for passthrough_env in (
                    ...
                    "WANDB_CACHE_DIR",
                    "WANDB_DATA_DIR",
                    "ROPD_SYNC_AFTER_BACKWARD",
                    "ROPD_SYNC_AFTER_OPTIM",
                    "ROPD_SYNC_VLLM_PHASES",
                    "ROPD_VALIDATE_ACTOR_BATCH",
                ):
                    if passthrough_env in os.environ:
                        env_vars[passthrough_env] = os.environ[passthrough_env]
                ...
                if rank == 0 and not precompute_master:
                    register_center_name = f"{self.name_prefix}_register_center"
                    register_center_wait_seconds = int(os.getenv("RAY_REGISTER_CENTER_WAIT_S", "300"))
                    register_center_actor = None
                    rank_zero_info = None
                    rank_zero_info_ref = worker.get_master_addr_port.remote()
                    for wait_idx in range(register_center_wait_seconds):
                        ready_refs, _ = ray.wait([rank_zero_info_ref], timeout=1)
                        if ready_refs:
                            master_addr, master_port = ray.get(ready_refs[0])
                            rank_zero_info = {"MASTER_ADDR": master_addr, "MASTER_PORT": master_port}
                            break

                        register_center_actor = self._get_named_actor(register_center_name)
                        if register_center_actor is not None:
                            rank_zero_info = ray.get(register_center_actor.get_rank_zero_info.remote())
                            break

                        if wait_idx % 30 == 29:
                            print(
                                f"[ray worker group] waiting for {register_center_name}; "
                                f"rank0_state={self._get_worker_state_summary(worker)}; "
                                f"actors={self._safe_list_named_actors()}",
                                flush=True,
                            )
