...
def main():
    ...
    if not ray.is_initialized():
        ...
        ray.init(include_dashboard=False, runtime_env={"env_vars": _ray_runtime_env_vars()})
...
def main_task(config: PPOConfig):
    ...
    reward_fn = CustomRewardManager(
        tokenizer=tokenizer,
        num_examine=config.worker.reward.num_examine,
        compute_score=config.worker.reward.compute_score,
        reward_config=config.worker.reward,
    )
