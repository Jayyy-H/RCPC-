...
try:
    ...
    FLAH_ATTN_CROSS_ENTROPY_LOSS_AVAILABLE = not _env_flag("VERL_DISABLE_FLASH_ATTN_CE", "0")
except ImportError:
...
def masked_mean(values, mask, axis=None, eps: float = 1e-8) -> torch.Tensor:
...
def pad_2d_list_to_length(response, pad_token_id, max_length=None) -> torch.Tensor:
    ...
    padded_response = [
        tuple(sub_list[:target_length]) + (pad_token_id,) * max(0, target_length - len(sub_list))
