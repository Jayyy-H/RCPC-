...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: RolloutConfig, tokenizer: PreTrainedTokenizer):
        ...
        self._mm_uuid_prefix = "verl-vllm-rollout"
    ...
    def _generate_sequences(self, prompts: DataProto, include_targets: bool, **kwargs) -> DataProto:
        ...
        if repeat_count > 1:
            ...
            attention_mask = _repeat_interleave(attention_mask, repeat_count)
            position_ids = _repeat_interleave(position_ids, repeat_count)
            if "pixel_values" in non_tensor_batch.keys():
                non_tensor_batch["pixel_values"] = _repeat_interleave(non_tensor_batch["pixel_values"], repeat_count)
                non_tensor_batch["image_grid_thw"] = _repeat_interleave(
                    non_tensor_batch["image_grid_thw"], repeat_count
                )
            if "images" in non_tensor_batch.keys():
                non_tensor_batch["images"] = _repeat_interleave(non_tensor_batch["images"], repeat_count)
            if "image_paths" in non_tensor_batch.keys():
                non_tensor_batch["image_paths"] = _repeat_interleave(non_tensor_batch["image_paths"], repeat_count)
            if "answer" in non_tensor_batch.keys():
                non_tensor_batch["answer"] = _repeat_interleave(non_tensor_batch["answer"], repeat_count)
            if "raw_prompt" in non_tensor_batch.keys():
                non_tensor_batch["raw_prompt"] = _repeat_interleave(non_tensor_batch["raw_prompt"], repeat_count)
